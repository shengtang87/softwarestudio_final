...
@ccclass
export default class player_cadence extends cc.Component {
    ...
    @property(cc.Prefab)
    private shield_prefab:cc.Prefab = null;
    ...
    update (dt) {
        ...
        if(this.havebuff){
            this.buff.x = this.node.x;
            this.buff.y = this.node.y;
        }
    }
    onKeyDown(e){
        if(this.node.name == "player1" || this.node.name == "player3"){
            if(e.keyCode == cc.macro.KEY.a){
                ...
                if(this.node.name == "player1"){
                    cc.find("Canvas/UI/UIMgr").getComponent("UIMgr").player1_mp_minus(2);
                }
                else if(this.node.name == "player3"){
                    cc.find("Canvas/UI/UIMgr").getComponent("UIMgr").player3_mp_minus(2);
                }
            }
        }
            ...
            if(e.keyCode == cc.macro.KEY.left){
                ...
                if(this.node.name == "player2"){
                    cc.find("Canvas/UI/UIMgr").getComponent("UIMgr").player2_mp_minus(2);
                }
                else if(this.node.name == "player4"){
                    cc.find("Canvas/UI/UIMgr").getComponent("UIMgr").player4_mp_minus(2);
                }
                this.shield();
            }
        }
    ...
    track(){
        if(this.state == 6 && this.holdtype == 0){
            ...
            this.graphics.clear();
            if(!this.havebuff) this.speed = 150;
            else this.speed = 250;
        }
    }
    straightdir(){
    ...
    shield(){
        cc.audioEngine.playEffect(this.fast_sound,false)//new
        var shield = cc.instantiate(this.shield_prefab);
        //shield.getComponent('magic_field').player = this.node;
        this.buff = shield;
        cc.find('Canvas').addChild(shield);
        this.havebuff = true;
        this.scheduleOnce(()=>{
            shield.destroy();
            this.havebuff = false;
