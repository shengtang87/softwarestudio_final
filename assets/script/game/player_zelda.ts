...
@ccclass
export default class player_zelda extends cc.Component {
    ...
    @property(cc.Prefab)
    private teleport_effect:cc.Prefab = null;
    ...
    private haveball: boolean = false;
    private isrush: boolean = false;
            ...
            if(e.keyCode == cc.macro.KEY.left){
                ...
                if(this.mana >= 1){
                    ...
                    if(this.node.name == "player2"){
                        ...
                        cc.find("Canvas/UI/UIMgr").getComponent("UIMgr").player4_mp_minus(1);
                    }
                }
            }
        }
    ...
    teleport(dir){
        cc.audioEngine.playEffect(this.rush_sound,false)//new

        var tp = cc.instantiate(this.teleport_effect);
        tp.zIndex = 2;
        tp.x = this.node.x;
        tp.y = this.node.y + 10;
        cc.find('Canvas').addChild(tp);
        this.scheduleOnce(()=>{tp.destroy();}, 0.55);

        this.unschedule(this.disRush);
        var rad = 0.25 * dir * Math.PI;
        var posX = 500 * Math.cos(rad);
        var posY = 500 * Math.sin(rad);
        this.isrush = true;
        this.rb.linearVelocity = cc.v2(posX,posY);
        this.scheduleOnce(this.disRush, 0.3);
    }
    disRush(){
        this.isrush = false;
    }
    /*
    teleport(dir){
        var tpa = cc.instantiate(this.teleport_effect);
        let state = tpa.getComponent(cc.Animation).play();
        state.wrapMode = cc.WrapMode.Reverse;
        tpa.zIndex = 2;
        tpa.x = this.node.x;
        tpa.y = this.node.y + 10;
        cc.find('Canvas').addChild(tpa);
        this.scheduleOnce(()=>{tpa.destroy();}, 0.55);

        var rad = 0.25 * dir * Math.PI;
        var posX = 100 * Math.cos(rad);
        var posY = 100 * Math.sin(rad);
        this.node.x += posX;
        this.node.y += posY;
        ...
        //this.isrush = true;
        //this.rb.linearVelocity = cc.v2(posX,posY);
        //this.scheduleOnce(()=>{this.isrush = false;}, 0.3);
    }*/
}
